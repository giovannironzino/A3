...
export const PatientWorkloadStep: React.FC<PatientWorkloadStepProps> = ({
  ...
  return (
    <div className="bg-[var(--branco)] border border-[var(--border-default)] p-6 sm:p-8 space-y-6 animate-fadeIn">
      {/* Header Badge */}
      <div className="flex items-center justify-between pb-3 border-b border-[var(--border-default)]">
        <Tag tone="diagnostico" className="flex items-center gap-1">
      ...
      <div className="bg-[var(--preto)] text-white p-6 sm:p-8 border-2 border-[var(--exodo-red)] space-y-4 relative overflow-hidden">
        <div className="flex items-center gap-2 text-[var(--exodo-red)] text-xs font-subtitle font-bold uppercase tracking-widest">
        ...
        <h2 className="font-display text-2xl sm:text-3xl text-white leading-tight">
          ...
          <span className="text-[var(--exodo-red)] font-extrabold underline decoration-[var(--exodo-red)] underline-offset-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 text-xs font-subtitle text-neutral-300 border-t border-neutral-800">
          ...
          <div className="flex items-center gap-2">
            <BarChart3 className="w-4 h-4 text-[var(--exodo-red)]" />
      ...
      {/* Explanation Note */}
      <Callout label="De onde vem esse cálculo?">
      ...
      <div className="space-y-3">
        ...
        <div className="border-2 border-[var(--preto)] overflow-x-auto">
          <table className="w-full text-left border-collapse min-w-[550px]">
            ...
            <tbody className="divide-y divide-[var(--border-default)] text-xs font-body">
              {productCalculations.map(({ product, monthlyHoursPerPatient, totalWeeklyHoursForProduct }) => (
                <tr key={product.id} className="hover:bg-[var(--cinza-claro)]">
                  <td className="p-3 border-r border-[var(--border-default)] font-subtitle font-bold text-[var(--preto)]">
                    {product.name}
                    <span className="block text-[0.65rem] font-normal text-[var(--cinza-medio)]">{product.format} • {product.durationLabel}</span>
                  </td>
                  <td className="p-3 border-r border-[var(--border-default)] font-subtitle font-bold text-[var(--exodo-red)]">
                    {product.activePatients} paciente(s)
                  </td>
                  <td className="p-3 border-r border-[var(--border-default)] text-[var(--cinza-escuro)]">
      ...
      <div className="flex justify-between items-center pt-4 border-t border-[var(--border-default)]">
        <button
          type="button"
          onClick={onNavigateToProducts}
          className="text-xs font-subtitle font-bold text-[var(--cinza-escuro)] hover:text-[var(--preto)] cursor-pointer border-none bg-transparent flex items-center gap-1"
        >
